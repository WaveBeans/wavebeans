...
private class ByteArrayFileOutputMock(
    ...
    private val uri = URI("test://${Random.nextLong().absoluteValue.toString(36)}.tmp")
    ...
    override fun writer(sampleRate: Float): Writer {
        return object : AbstractWriter<Sample>(
            stream,
            sampleRate,
            FileWriterDelegate(
                uriGenerationStrategy = { uri },
                localFileFactory = TestWbFileDriver.driver
            ),
            ByteArrayFileOutputMock::class
        ) {
    ...
    fun readBytes(sampleRate: Float): List<Byte> {
        ...
        writer.close()
        return TestWbFileDriver.driver.fs.getValue(uri.toString()).toList()
    }
}
    ...
    describe("Wav LE input, sample rate = 50.0Hz, bit depth = 8, mono") {
        ...
        describe("output based on that input") {
            ...
            it("should return byte array as input stream the same as initial buffer") {
                assertThat(
                    output.readBytes(sampleRate).map { it.toInt() }).isEqualTo(buffer.map { it.toInt() and 0xFF })
            }
        }

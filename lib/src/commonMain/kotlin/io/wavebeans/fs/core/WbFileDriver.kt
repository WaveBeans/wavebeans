...
interface WbFileDriver {
    ...
    companion object {
        ...
        // TODO move to config
        var defaultLocalFileScheme = "file"

        fun defaultLocalFileFactory() = instance(defaultLocalFileScheme)

        private val registry = ConcurrentHashMap<String, WbFileDriver>()
        ...
        fun instance(scheme: String): WbFileDriver =
                registry[scheme.toLowerCase()]
                        ?: throw IllegalArgumentException("Scheme `$scheme` can't be found among registered $registry")

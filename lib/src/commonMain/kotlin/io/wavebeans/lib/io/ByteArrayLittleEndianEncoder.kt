...
fun ByteArray.encodeSampleLEBytes(at: Int, element: Sample, bitDepth: BitDepth) {
    require(this.size - at >= bitDepth.bytesPerSample) { "At least ${bitDepth.bytesPerSample} required but ${this.size - at} available" }
    if (bitDepth != BitDepth.BIT_8) {
        ...
        (0 until bitDepth.bytesPerSample)
                ...
                .forEachIndexed { j: Int, v: Byte ->
                    this[at + j] = (v.toInt() and 0xFF).toByte()
                }
    } else {

...
) : AbstractOperationBeanStream<Pair<Long, Window<Sample>>, FftSample>(input),
    AlterBean<Pair<Long, Window<Sample>>, FftSample> {

    override fun operationSequence(
        input: Sequence<Pair<Long, Window<Sample>>>,
        sampleRate: Float
    ): Sequence<FftSample> {
        return input
            .map { (index, window) ->

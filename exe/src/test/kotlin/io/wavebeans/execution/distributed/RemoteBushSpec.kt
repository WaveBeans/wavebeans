...
class RemoteBushSpec : DescribeSpec({
    ...
    describe("Properly working bush") {
        val bushKey by lazy { newBushKey() }

        val remoteBush by lazy {
    ...
    describe("Pointing to non-existing bush") {
        // no bush
        val bushKey by lazy { newBushKey() }
        val remoteBush by lazy { RemoteBush(bushKey, "127.0.0.1:$communicatorPort") }

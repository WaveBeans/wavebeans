        ...
        describe("Mode: ${mode.key}") {
                ...
                describe("Iterate over pod sequence") {
                    newTestSplittingPod(seq, 2).use { pod ->
                        ...
                        describe("Partition 0") {
                            val result = sampleRemap(pod.iteratorNext(iteratorKey0, 50))
                                    ?.map { it.asInt() }
                        ...
                        describe("Partition 1") {
                            val result = sampleRemap(pod.iteratorNext(iteratorKey1, 50))
                                    ?.map { it.asInt() }
            ...
            describe("SplittingPod returning predefined sequence. One partition") {
                describe("Iterate over pod sequence") {
                    newTestSplittingPod(seq, 1).use { pod ->
                        ...
                        describe("Partition 0") {
                            val result = sampleRemap(pod.iteratorNext(iteratorKey0, 100))
                                    ?.map { it.asInt() }
            ...
            describe("SplittingPod returning predefined sequence. Three partitions") {
                describe("Iterate over pod sequence") {
                    newTestSplittingPod(seq, 3).use { pod ->
                        ...
                        describe("Partition 0") {
                            val result = sampleRemap(pod.iteratorNext(iteratorKey0, 34))
                                    ?.map { it.asInt() }
                        ...
                        describe("Partition 1") {
                            val result = sampleRemap(pod.iteratorNext(iteratorKey1, 33))
                                    ?.map { it.asInt() }
                        ...
                        describe("Partition 2") {
                            val result = sampleRemap(pod.iteratorNext(iteratorKey2, 33))
                                    ?.map { it.asInt() }
            ...
            describe("Iterate over pod sequence with 2 proxies. Two partitions") {
                newTestSplittingPod(seq, 2).use { pod ->
                    ...
                    describe("Partition 0") {
                        val result1 = sampleRemap(pod.iteratorNext(iteratorKey1p0, 50))
                                ?.map { it.asInt() }
                        val result2 = sampleRemap(pod.iteratorNext(iteratorKey2p0, 50))
                                ?.map { it.asInt() }
                    ...
                    describe("Partition 1") {
                        val result1 = sampleRemap(pod.iteratorNext(iteratorKey1p1, 50))
                                ?.map { it.asInt() }
                        val result2 = sampleRemap(pod.iteratorNext(iteratorKey2p1, 50))
                                ?.map { it.asInt() }
            ...
            describe("Iterate over pod sequence for more than defined in sequence at once. Two partitions") {
                newTestSplittingPod(seq, 2).use { pod ->
                    ...
                    val iteratorKey1 = pod.iteratorStart(100.0f, 1)
                    describe("Partition 0") {
                        val e = sampleRemap(pod.iteratorNext(iteratorKey0, 51))
                                ?.map { it.asInt() }
                    ...
                    describe("Partition 1") {
                        val e = sampleRemap(pod.iteratorNext(iteratorKey1, 51))
                                ?.map { it.asInt() }
            ...
            describe("Iterate over pod sequence for more than defined in sequence in two attempts. Two partitions") {
                newTestSplittingPod(seq, 2).use { pod ->
                    ...
                    describe("Partition 0") {
                        val e1 = sampleRemap(pod.iteratorNext(iteratorKey0, 50))
                                ?.map { it.asInt() }
                        val e2 = sampleRemap(pod.iteratorNext(iteratorKey0, 1))
                                ?.map { it.asInt() }
                    ...
                    describe("Partition 1") {
                        val e1 = sampleRemap(pod.iteratorNext(iteratorKey1, 50))
                                ?.map { it.asInt() }
                        val e2 = sampleRemap(pod.iteratorNext(iteratorKey1, 1))
                                ?.map { it.asInt() }

...
/**
 * Transferring type for array of [SampleArray] to keep [Sample]s inside [Window] within primitive [Double] during transfer.
 */
data class WindowSampleArray(
        val windowSize: Int,
        val windowStep: Int,
        val sampleArray: Array<SampleArray>
)
 ...
 */
fun createWindowSampleArray(size: Int, windowSize: Int, windowStep: Int, windowFn: (Int) -> Window<Sample>): WindowSampleArray {
    require(size >= 1) { "Can't create sample array of the size $size" }
    return WindowSampleArray(windowSize, windowStep, Array(size) {

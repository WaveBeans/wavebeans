...
internal val protoBuf = ProtoBuf { this.encodeDefaults = false }

internal fun <T : Any> T.asByteArray(serializer: KSerializer<T>): ByteArray =
    try {
        val buf = protoBuf.encodeToByteArray(serializer, this)
...
internal fun <T> ByteArray.asObj(serializer: KSerializer<T>): T =
    try {
        ...
        protoBuf.decodeFromByteArray(serializer, buf)
    } catch (e: Throwable) {
        if (e is OutOfMemoryError) throw e // most likely no resources to handle. Just fail
        throw IllegalStateException(
            "Can't deserialize with `$serializer` the buffer " +

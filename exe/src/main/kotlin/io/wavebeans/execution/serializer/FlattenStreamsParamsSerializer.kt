...
object FlattenStreamsParamsSerializer : KSerializer<FlattenStreamsParams<*, *>> {
    override val descriptor: SerialDescriptor =
        buildClassSerialDescriptor(FlattenStreamsParams::class.className()) {
            ...
            element("map", String.serializer().descriptor)
        }
        ...
        return decoder.decodeStructure(descriptor) {
            ...
            loop@ while (true) {
                when (val i = decodeElementIndex(descriptor)) {
                    ...
                    1 -> map = decodeStringElement(descriptor, i)
                    else -> throw SerializationException("Unknown index $i")
                }
            }
            FlattenStreamsParams<Any, Any>(scope, lambdaWrapper.deserialize2(map))
        }
    }
    ...
    override fun serialize(encoder: Encoder, value: FlattenStreamsParams<*, *>) {
        encoder.encodeStructure(descriptor) {
            ...
            encodeStringElement(descriptor, 1, lambdaWrapper.serialize(value.map))
        }
    }
}
